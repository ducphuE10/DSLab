...
class DataReader:
    def __init__(self, data_path, batch_size, vocab_size):
        ...
        for data_id, line in enumerate(d_lines):
            ...
            print(len(tokens), sentence_length)
    ...
    def next_batch(self):
        ...
        return self._data[start:end], self._label[start:end], self._sentence_lengths[start:end], self._final_tokens[start:end]
class RNN:
    def __init__(self,
                 vocab_size,
                 embedding_size,
    ...
    def embedding_layer(self, indices):
        pretrained_vector = [np.zeros(self._embedding_size)]
        np.random.seed(2018)
        for _ in range(self._vocab_size + 1):
            pretrained_vector.append(np.random.normal(loc=0., scale=1., size=self._embedding_size)) #loc ~mean, scale ~var
...
def train_and_evaluate_RNN():
    ...
    with tf.Session() as sess:
        train_data_reader = DataReader(data_path='../datasets/w2v/20news-train-encoded.txt', batch_size=50
                                       , vocab_size=vocab_size)

        test_data_reader = DataReader(data_path='../datasets/w2v/20news-test-encoded.txt', batch_size=50
                                      , vocab_size=vocab_size)
        ...
        while step < MAX_STEP:
            train_data, train_labels, train_sentence_lengths, train_final_tokens = train_data_reader.next_batch()
            ...
            if train_data_reader._batch_id == 0:
                num_true_preds = 0
                while True:
                    test_data, test_labels, test_sentence_lengths,test_final_tokens = test_data_reader.next_batch()
                    ...
                    if test_data_reader._batch_id == 0: # duyệt hết 1 epoch -> dừng
                        break
...
#Epoch:  3
#Accuracy on test data:  97.68428495669083